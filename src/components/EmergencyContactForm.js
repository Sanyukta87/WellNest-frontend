...
function EmergencyContactForm() {
  ...
  useEffect(() => {
    const fetchContact = async () => {
      try {
        ...
        const res = await axios.get("https://wellnest-backend-3911.onrender.com/api/profile/me", {
  ...
  const handleSave = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.put(
        "https://wellnest-backend-3911.onrender.com/api/profile/emergency-contact",
